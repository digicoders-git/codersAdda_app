...
class _GeneralTestsPageState extends State<GeneralTestsPage> with SingleTickerProviderStateMixin {
  ...
  void _openInstructions(Map<String, dynamic> quiz) async {
    ...
    if (scheduledTime != null && scheduledTime.isAfter(DateTime.now())) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("Test hasn't started yet. Starts on: $formatted"),
  ...
  void _joinWithCode(String code) {
    ...
    final quiz = _availableQuizzes.firstWhere(
      (q) {
        final c1 = (q['testCode'] ?? '').toString().trim().toLowerCase();
        final c2 = (q['quizCode'] ?? '').toString().trim().toLowerCase();
    ...
    if (quiz.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text("Invalid test code. Please check and try again."),
          backgroundColor: Colors.red.shade700,
        ),
      );
      return;
  ...
  Future<void> _fetchQuizzes() async {
    try {
      final response = await _apiClient.get('${ApiUrls.getQuizzes}?courseId=general&type=Test');
      if (response != null && response['success'] == true) {
        final List<dynamic> data = response['data'] ?? [];
        if (mounted) {
          setState(() {
            _availableQuizzes = data.map((q) => {
              ...
              'points': q['points'] ?? 0,
              'testCode': q['testCode'] ?? q['quizCode'] ?? '',
              'quizCode': q['quizCode'] ?? q['testCode'] ?? '',
              'type': 'Test',
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Row(
                children: [
                  _buildTabPill('Available Tests', 0),
                  const SizedBox(width: 4),
                  _buildTabPill('Attempted Tests', 1),
                ],
              ),
            ),
                      ...
                      child: ElevatedButton(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Text(
                              isLocked ? 'Scheduled Test' : 'Start Test',
                              style: TextStyle(
                                fontSize: AppSizer.deviceSp13,
                                fontWeight: FontWeight.bold,
                              ),
